...
class Collection extends Event{
    add(data){
        ...
        data.forEach(function(val){
            ...
            this.emit('add', {
                element: val, index: this.__zzArray.length-1, target: this
            });
        }, this);
        
        this.emit('add-values', {
            values: data, target: this
        });
    ...
    removeAll(){
        ...
        all.forEach(function(val, idx){
            this.emit('remove', {
                element: val, index: idx, target: this
            });
        }, this);
        
        this.emit('remove-values', {
            values: all, target: this
        });
    ...
    splice(index, count){
        ...
        for (let i in val){
            this.emit('remove', {
                element: val[i], index: index+i, target: this
            });
        }
    }
    ...
    __zzRemoveSilence(val){
        let i = this.indexOf(val);
        if (i !== -1){
            this.__zzArray.splice(i, 1);
        }
...
class CollectionFilter extends Event{
    ...
    __zzSendChanges(collection, elements){
        if (!Array.isArray(elements)){
            console.error('CollectionFilter is not filter function');
    ...
    to(collection){
        ...
        collection.replace( this.filter( this.collection.elements ) );
    ...
    constructor(collection){
        ...
        this.setFilterFn(this.filter);
        
        this.collection = collection;
        this.collection.on('add', this.__zzOnChange, this);
        this.collection.on('remove', this.__zzOnChange, this);
        this.collection.on('replace-values', this.__zzOnChange, this);
    }
}
