...
class EventsGroup{
    add(listener, once, prepend){
        ...
        if (listener instanceof EventListener){
            ...
            listener.addGroup(this);
        }
    }
    
    remove(listener){
        if (listener instanceof EventListener){
            i = this.once.indexOf(listener);
            (i !== -1) && this.once.splice(i, 1);
            
            let i = this.many.indexOf(listener);
...
class Event{
            ...
            if (typeof name === 'string'){
                ...
                if (typeof name === 'function'){
                    for (let evName in events){
                        events[evName].removeFn(name);
                    }
                }else{
                    for (let evName in events){
                        events[evName].removeBySelf(name);
                    }
                }
            }
        }
    ...
    emit(name){
        ...
        evGroup.emit([].slice(arguments, 1));
...
module.exports = {EventStack, EventListener, EventsGroup, Event};
